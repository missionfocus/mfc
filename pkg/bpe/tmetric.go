...
import (
	"fmt"
	"git.missionfocus.com/ours/code/libraries/go/tmetric/client"
...
func GetTimeEntriesWithParams(vaultClient vault.Vault, params *time_entries.TimeEntriesGetTimeEntriesParams) ([]*models.TimeEntry, error) {
	_, auth := tmetric.GetTmetricAuth(vaultClient)
	resp, err := client.Default.TimeEntries.TimeEntriesGetTimeEntries(params, auth)
	if err != nil {
		panic(err)
	}
	...
	for _, m := range scope.Members {
		...
		if person == "" || strings.ToLower(m.UserProfile.UserName) == strings.ToLower(person) || emailPointerValue == person+"@missionfocus.com" {
			...
			for _, t := range calRange {
				...
				timeEntries, _ := GetTimeEntriesWithParams(vaultClient, params)
